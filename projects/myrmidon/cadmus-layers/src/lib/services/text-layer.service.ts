...
export class TextLayerService {
  ...
  private renderTokenAtEnd(pt: TokenPoint, token: string, sb: string[]): void {
    if (pt.at && pt.at > 0) {
      ...
      sb.push('</span>');
      // ...ken
      if (pt.at - 1 + (pt.run || 1) < token.length) {
        sb.push(token.substr(pt.at - 1 + (pt.run || 1)));
      }
    } else {
      // ...token]
      sb.push(token);
  ...
  private renderTokenAtPointStart(
    ...
    // 1.token's left-part
    if (loc.primary.at && loc.primary.at > 0) {
      // to...
  ...
  public render(
    ...
    selectedLoc: TokenLocation | null = null
  ): string {
    if (!text) {
      return '<div></div>';
    }
    ...
    let loc: TokenLocation | undefined;
  ...
  */
  ...
  public getSelectedLocationForEdit(
    ...
    return span
      ? this.getSpanLocFromId(span.getAttribute('id') as string)
      : null;
  }
